...
function App() {
  ...
  const fetchData = async (location: string, activity: string): Promise<any> => {
    ...
    // let newData = transFormAddresses(JSON.parse(json).results);
    if(JSON.parse(json).status == "ZERO_RESULTS") {
      setNoResults(true);
    } else {
      setNoResults(false);
    }
    setUserResults(JSON.parse(json).results);
    setIsLoading(false);
  }
    ...
    <div className="App">
      ...
      <div className="input-results">
        {isLoading ? 
          <p>Loading...</p>
          :
          <LocationList zeroResults={noResults} list={userResults} />
        }
      </div>
    </div>
  );
